...
class CartRemove(EventViewMixin, CartActionMixin, AsyncAction, View):
    ...
    def post(self, request, *args, **kwargs):
        if 'id' in request.POST:
            return self.do(self.request.event.id, request.POST.get('id'), self.request.session.session_key)
        else:
...
class CartClear(EventViewMixin, CartActionMixin, AsyncAction, View):
    ...
    def post(self, request, *args, **kwargs):
        return self.do(self.request.event.id, self.request.session.session_key)
